...
class Inference_Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            print("len features: ", len(flat_inputs))
            print("self.inputs['xyz'].shape", self.inputs['xyz'][0].shape)
            print("self.inputs['neigh_idx'].shape", self.inputs['neigh_idx'][0].shape)
            print("self.inputs['sub_idx'].shape", self.inputs['sub_idx'][0].shape)
            print("self.inputs['interp_idx'].shape", self.inputs['interp_idx'][0].shape)
            print("self.inputs['features'].shape", self.inputs['features'].shape)
        ...
        # self.saver.save(self.sess,  'graph_analysis', global_step=self.training_step)
        # exit()
    ...
    #     self.sess.run(dataset.train_init_op, options=self.run_opts)
    ...
    #                    self.labels,
    #                    self.accuracy]
    #             _, _, summary, l_out, probs, labels, acc = self.sess.run(ops, {self.is_training: True}, options=self.run_opts)
    ...
    def test_savedmodel(self, dataset):

        ops = (self.prob_logits)
        stacked_prob = self.sess.run(ops, {self.is_training: False})
        pred = np.argmax(stacked_prob, 1)
        return pred
